...
function start_cp1() {
    ...
    local num=$(ps_func | grep -w $CP1 | grep "$proxy" | grep -w "$key" | grep -v grep | wc -l)
    if (( num == 0 )); then
...
function start_cp2() {
    ...
    local proxy="$3"
    local listen="$4"
    local num=$(ps_func | grep -w $CP2 | grep "$proxy" | grep -w "$key" | grep -v grep | wc -l)
    if (( num == 0 )); then
